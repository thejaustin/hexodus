...
fun FeatureExplorerScreen(
    ...
    val baseFeatures = when (category) {
    ...
    val features = remember(baseFeatures, sortBy) {
        when (sortBy) {
            "Name" -> baseFeatures.sortedBy { it.title }
            "Status" -> baseFeatures.sortedBy { it.status.name }
            else -> baseFeatures
        }
    }
            ...
            TopAppBar(
                ...
                actions = {
                    ...
                    DropdownMenu(
                        ...
                        listOf("Name", "Status").forEach { option ->
                            DropdownMenuItem(
                                text = { Text(option) },
                                onClick = {
                                    sortBy = option
        ...
        if (features.isEmpty()) {
            Box(
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Icon(Icons.Default.Explore, null, modifier = Modifier.size(48.dp), tint = MaterialTheme.colorScheme.secondary)
                    Spacer(modifier = Modifier.height(12.dp))
                    Text("No features here yet.", style = MaterialTheme.typography.titleMedium)
                }
            }
        } else {
        LazyColumn(
            ...
            groupedFeatures.forEach { (status, featuresInStatus) ->
                item {
                    Text(
                        text = status.name,
                ...
                itemsIndexed(featuresInStatus, key = { _, feature -> feature.title }) { index, feature ->
                    val key = feature.featureKey

                    AnimatedVisibility(
                        visible = visible,
                        enter = slideInHorizontally(animationSpec = tween(400, delayMillis = index * 30)) { -it / 10 } + 
