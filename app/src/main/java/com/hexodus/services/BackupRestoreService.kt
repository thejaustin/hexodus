...
object BackupRestoreService {
    ...
    private fun backupTheme(themeName: String, includeSettings: Boolean) {
        try {
            ...
            val backupFile = File(HexodusApplication.context.getExternalFilesDir(null), "backups/themes/${themeName}_backup.zip")
    ...
    private fun restoreTheme(backupPath: String) {
        try {
            ...
            HexodusApplication.context.getExternalFilesDir(null)?.parent?.let { allowedPaths.add(it) }
            HexodusApplication.context.cacheDir.parent?.let { allowedPaths.add(it) }
    ...
    private fun backupSettings() {
        try {
            val prefs = HexodusApplication.context.getSharedPreferences("hexodus_prefs", 0)
            Log.d(TAG, "Backing up settings")
            HexodusApplication.context.sendBroadcast(Intent("SETTINGS_BACKUP_COMPLETED"))
        } catch (e: Exception) {
    ...
    private fun restoreSettings(backupPath: String) {
        try {
            ...
            HexodusApplication.context.sendBroadcast(Intent("SETTINGS_RESTORE_COMPLETED"))
        } catch (e: Exception) {
    ...
    private fun getBackupList() {
        try {
            val backupDir = File(HexodusApplication.context.getExternalFilesDir(null), "backups")
