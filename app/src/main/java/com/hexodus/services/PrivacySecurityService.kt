...
object PrivacySecurityService {
    ...
    private fun lockApp(targetPackageName: String, lockMethod: String) {
        try {
            ...
            val prefs = HexodusApplication.context.getSharedPreferences("app_locks", 0)
    ...
    private fun hideFile(filePath: String) {
        try {
            // Validate file path
            if (!SecurityUtils.isValidFilePath(filePath, listOf(HexodusApplication.context.filesDir.parent, HexodusApplication.context.cacheDir.parent).filter { it != null })) {
            ...
            // Store in preferences
            val prefs = HexodusApplication.context.getSharedPreferences("hidden_files", 0)
            val editor = prefs.edit()
