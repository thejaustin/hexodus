...
object SystemTunerService {
    ...
    private fun modifySystemSetting(key: String, value: String, type: String) {
        scope.launch {
            try {
                ...
                ShizukuBridge.Settings.putSystem(key, value)
    ...
    private fun getSystemSetting(key: String) {
        scope.launch {
            try {
                if (!ShizukuBridge.isReady()) return@launch
                val result = ShizukuBridge.Settings.getSystem(key)
    ...
    private fun toggleImmersiveMode() {
        scope.launch {
            try {
                if (!ShizukuBridge.isReady()) return@launch
                val currentValue = ShizukuBridge.Settings.getSystem("immersive_mode")
                val newValue = if (currentValue.contains("true")) "false" else "true"
                ShizukuBridge.Settings.putSystem("immersive_mode", newValue)
